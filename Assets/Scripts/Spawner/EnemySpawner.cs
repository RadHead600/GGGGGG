...
{
    private const int InitialCountWaves = 1;
    private const int IncreaseEnemiesPercentage = 100;
    ...
    {
        _enemiesReloadingTime = SpawnerParameters.MaxReloadingTime - 
        Mathf.Pow(GameInformation.Instance.Information.PassedLevel, SpawnerParameters.ReduceReloadingTime);
        ...
        if (_spawnerCoroutine == null)
            _spawnerCoroutine = StartCoroutine(StartEnemiesSpawner((SpawnerEnemiesParameters)
            SpawnerParameters, _enemiesReloadingTime, InitialCountWaves));
    }

    private IEnumerator StartEnemiesSpawner(SpawnerEnemiesParameters parameters, float reloadingTime, int countWaves)
    {
        _objectsCount = parameters.MinCountObjects + (int)((float)parameters.MinCountObjects * 
        ((float)parameters.IncreaseEnemies / IncreaseEnemiesPercentage));
        ...
        _wavesToPassed = parameters.MinWaves + (int)((float)GameInformation.Instance.Information.PassedLevel / 
        (float)parameters.NumLevelForAddWaves);
